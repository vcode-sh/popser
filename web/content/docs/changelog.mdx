...
- **Animation duration customization.** `--popser-transition-duration` and `--popser-anchored-transition-duration` CSS custom properties. Pure CSS, zero JS. ([Sonner #690](https://github.com/emilkowalski/sonner/issues/690))
- **Popover API support.** Toast viewport uses `popover="manual"` + `showPopover()`. Toasts render in the top layer, above dialogs and modals. Progressive enhancement. ([Sonner #655](https://github.com/emilkowalski/sonner/issues/655), [#667](https://github.com/emilkowalski/sonner/issues/667))
- **Expanded limit.** `expandedLimit` prop on `<Toaster>`. Collapsed shows `limit`, hover expands to `expandedLimit`. ([Sonner #637](https://github.com/emilkowalski/sonner/issues/637))
- **Close button position.** `closeButtonPosition`: `"header"` (default) or `"corner"` (absolute top-right). Per-toast override supported. ([Sonner #747](https://github.com/emilkowalski/sonner/issues/747))
- **AbortSignal for promise toasts.** `signal`, `aborted`, and `onAbort` on `toast.promise()`. Cancel in-flight requests. Default aborted type: `"warning"`. ([Sonner #681](https://github.com/emilkowalski/sonner/issues/681))
- **Entry direction.** `enterFrom`: `"top"` | `"bottom"` | `"left"` | `"right"`. Per-toast animation direction override. ([Sonner #671](https://github.com/emilkowalski/sonner/issues/671))
- **RTL support.** `dir` prop on `<Toaster>`: `"ltr"` | `"rtl"` | `"auto"`. Flips positions, swipe directions, and animations.
- **Toast history.** `toast.getHistory()` and `toast.clearHistory()`. Opt-in ring buffer via `historyLength` prop. `ToastHistoryEntry` type exported.

### Changed

- 7 CSS files updated for new features.
- `types.ts` expanded with new option interfaces.
- `index.ts` barrel export updated with `ToastHistoryEntry`.
...
- shadcn registry served from the docs site. Direct install: `npx shadcn add https://popser.vcui.dev/r/popser.json`.
- Namespace install: configure `@vcode-sh` in your `components.json`, then `npx shadcn add @vcode-sh/popser`.
- `/llms.txt` — concise index of all docs pages with links and descriptions.
- `/llms-full.txt` — full documentation concatenated into one plain text file. JSX demo components stripped out.

### Fixed

- shadcn docs referenced `<Popser>` but the registry component exports `<Toaster>`. Fixed everywhere.
- README CSS examples used HSL values. The token system is OKLCH. Now they match.
- Factual accuracy sweep across all docs: corrected @import file count (6 not 7), Base UI release date (late 2025 not early 2026), Sonner classNames count, update() API comparison, CSS bundling description, migration example, and removed unverifiable quotes.
...
- [Why I built this](/why) — the honest version of the README's "Why" section.
- [Why Base UI, not Radix](/why-base-ui) — the technical argument for headless primitives in 2026.
- [Collaborate](/collaborate) — contribution guide with actual context, not just a checklist.
...
- `README.md` rewritten with bundle size table, docs site links, and contributing section.
- `CONTRIBUTING.md` rewritten with architecture reference and human tone.
- `package.json` homepage now points to popser.vcui.dev.
- Comparison docs (`popser-sonner.md`, `popser-toast.md`) updated with v1.1.0 bundle sizes.

...
| Type | What it does |
|------|-------------|
| `Element` | DOM element — ref, querySelector, whatever |
...
Toast appears right where the click happened. Pass the native event.

<AnchorClickDemo />

```ts
function handleClick(event: React.MouseEvent) {
  toast.success("Right here", {
...
Control where the toast sits relative to the anchor. Try all four sides:
...
Show an arrow pointing at the anchor. Compare with and without:

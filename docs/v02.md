...
├── 1.5  Generic manager type                    [1 line]
├── 1.6  Optimize untrackToast                   [~20 lines]
├── 1.3  Document onRemove                       [docs only]
├── 1.2  Error boundary logging                  [~5 lines]
├── 1.1  Fix promise undefined return            [~15 lines]
└── 1.4  Fix data field collision                [~30 lines]

Phase 2 (core — v0.2.0-alpha)
├── 2.1  toast.custom(jsx)                       [~80 lines]
├── 2.2  Enhanced toast.promise()                [~100 lines]
├── 2.3  Per-toast classNames                    [~40 lines]
├── 2.4  Per-toast unstyled                      [~20 lines]
└── 2.6  Action/Cancel as ReactNode              [~30 lines]

Phase 3 (enhanced — v0.2.0-beta)
├── 3.1  toast.message()                         [~3 lines]
├── 3.2  Per-toast richColors                    [~15 lines]
├── 3.4  Callback receives toast ID              [~20 lines]
├── 3.5  Toast.Positioner (anchored toasts)      [~60 lines]
├── 3.6  data-limited exit styling               [CSS only]
└── 3.7  Direction-aware swipe exit              [CSS only]

Phase 4 (polish — v0.2.0)
├── 4.1  toastOptions prop                       [~30 lines]
├── 4.2  mobileOffset prop                       [~15 lines]
├── 4.3  Documentation update                    [docs]
└── 4.4  shadcn registry update                  [docs]
```

**Estimated total:** ~460 lines of source + ~200 lines of tests + docs.
...
- [ ] All v0.1 tests pass unchanged (backward compat)
- [ ] Coverage stays above 90% on all thresholds
- [ ] `toast.custom()` works with Sonner migration patterns
- [ ] Enhanced promise accepts all Sonner promise patterns
- [ ] No new `!important` in CSS
- [ ] No new peer dependencies
- [ ] Docs fully updated
- [ ] shadcn registry component updated
- [ ] Type-check passes with `verbatimModuleSyntax` and strict mode
